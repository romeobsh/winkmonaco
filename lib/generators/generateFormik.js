...
export const generateFormik = (schema, endpoint, title, id) => {
  const FormikWrapper = ({ children }) => {
    ...
    const [initialValues, setInitialValues] = useState(generateInitialValues(schema));
    ...
    useEffect(() => {
      if (id) {
        const fetchData = async () => {
          try {
            const { data } = await (await fetch(`/api/${endpoint}/${id}`)).json();
      ...
      <React.Fragment>
        ...
        <form onSubmit={formik.handleSubmit}>
          <Grid container spacing={2} sx={{ width: "calc(100vw - 272px)", maxWidth: "1400px" }}>
            ...
            {!isLoading && (
              <React.Fragment>
                {generateFormFields(articleSchema, formik.values, formik.errors, formik.touched, formik.handleChange)}
                <Grid item xs={12}>
