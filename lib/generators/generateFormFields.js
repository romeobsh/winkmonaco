...
import { Grid, TextField, Checkbox, FormControlLabel, FormGroup, MenuItem, Typography } from "@mui/material";
import { Translate } from "@mui/icons-material";

export const generateFormFields = (schema, isSending, values, errors, touched, handleChange) => {
  return schema.map((field) => {
    const { name, title, placeholder, translation, muiHeaderName, muiType, muiMdSize, selectOptions, fullWidth, multiline, minRows, disabled, label } = field;
      ...
      return (
        <Grid item xs={12} md={muiMdSize ?? 12} key={name}>
          ...
          <TextField
            ...
            value={muiType === "string" || muiType === "number" ? values[name] || "" : new Date(values[name]).toLocaleDateString() || ""}
